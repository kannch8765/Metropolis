...
type ResourceRow = {
  ...
  accessibility_tags_json: string;
...
type MetricRow = {
  ...
  resources_per_1000_foreign_residents: number | null;
...
const seedResources: Resource[] = [
  {
    ...
    description: 'Seed record used until the first verified resource source is connected.',
    latitude: 35.6895,
    longitude: 139.6917,
    wardId: 'shinjuku',
    languages: ['ja', 'en', 'zh'],
...
async function searchResources(db: D1Database | undefined, filters: QueryFilters): Promise<Resource[]> {
  ...
  if (filters.ward_id) {
    clauses.push('ward_id = ?');
    bindings.push(filters.ward_id);
  }
  if (filters.kind) {
    clauses.push('kind = ?');
  ...
  const statement = db.prepare(`SELECT * FROM resources ${where} ORDER BY name LIMIT ?`).bind(...bindings, limit);
...
async function getAreaMetrics(db: D1Database | undefined, wardIds: string[]): Promise<WardMetric[]> {
  ...
  const result = await db.prepare(
    `SELECT
       ...
       COUNT(DISTINCT je.value) AS language_count,
       d.foreign_residents,
       CASE WHEN d.foreign_residents > 0
         THEN ROUND(COUNT(DISTINCT r.id) * 1000.0 / d.foreign_residents, 4)
         ELSE NULL END AS resources_per_1000_foreign_residents,
       d.source_url,
       d.reference_date AS source_updated_at,
...
async function handleToolQuery(request: Request, env: Env): Promise<Response> {
  ...
  switch (payload.action) {
    case 'search_resources':
      return json({ resources: await searchResources(env.DB, filters) });
    case 'get_resource':
      return json({ resource: await getResource(env.DB, filters.id) });
    case 'get_area_metrics':
      return json({ metrics: await getAreaMetrics(env.DB, filters.ward_id ? [filters.ward_id] : []) });
    case 'compare_areas':
      return json({ metrics: await getAreaMetrics(env.DB, filters.ids?.slice(0, 10) ?? []) });
    default:
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/api/resources' && request.method === 'GET') {
      ...
      return json({ resources, source: env.DB ? 'd1' : 'seed' });
    }

    if (url.pathname === '/api/tools/query' && request.method === 'POST') {
      return handleToolQuery(request, env);
    }
